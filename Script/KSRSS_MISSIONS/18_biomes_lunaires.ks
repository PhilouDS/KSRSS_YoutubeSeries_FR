...
local visitedBiome is  // -> Dépend des biomes visités
  list(
    "Lowlands",
    "North Pole",
    "South Pole",
...
correctionFutureInclinaison(90, 500, 0.05, 0.5, false).
wait 0.5.
exeMnv().
wait 0.5.
...
until wantedBiome:find(actual_biome) < 0 {
  set warp to 2.
  set actual_biome to addons:scanSat:getBiome(ship:body, ship:geoPosition).
